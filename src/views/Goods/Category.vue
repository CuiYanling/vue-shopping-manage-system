...
<template>
    <el-card class="box-card">
        ...
        <!-- 树形组件 -->
        <el-tree :data="categorylist" default-expand-all>
            <!-- scope 解构 { node, data }  -->
...
//加载所有商品分类
onMounted(async () => {
    let { status, msg, data } = await Goods.allcategory({ type: 'tree' });
    console.log(data);
    if (status) {
...
let handleEdit = (formEl) => {
    formEl.validate(async (valid) => {
        ...
        if (valid) {
            let { status, msg, data } = await Goods.categoryedit(edit_form.value.id, { ...edit_form.value });
            if (status) {
                //编辑成功
                // console.log(data);
                //更新DOM
...
let handleAdd = (formEl) => {
    formEl.validate(async (valid) => {
        if (valid) {
            let { status, msg, data } = await Goods.categoryadd({ ...insert_form, pId: current_data.value.id, img: current_data.value.img });
            if (status) {
...
let handleRemove = (node, data) => {
    ElMessageBox.confirm('此操作将永久删除该分类，确定删除嘛？', { type: 'warning' })
        .then(async () => {
            let { status, msg } = await Goods.categorydelete(data.id, { id: data.id });
            if (status) {
                //删除成功，操作DOM
