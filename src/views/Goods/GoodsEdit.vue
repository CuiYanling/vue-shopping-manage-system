    ...
    <el-card>
        ...
        <el-form ref="ruleFormRef" :rules="rules" label-width="120px" label-position="left" :model="form"
            ...
            <el-form-item label="折扣" prop="discount">
                <div>
                    <el-input v-model="form.discount" style="width:200px" disabled>
            ...
            <el-form-item label="商品主图720" prop="img_lg">
                <el-upload class="avatar-uploader" :action="`${host}/upload/common`" :data="extra"
                    :headers="headers" :show-file-list="false" accept=".jpg,.png,.jpeg"
                    :on-success="handleUploadSuccess1" :on-error="handleUploadError1"
            ...
            <el-form-item label="商品主图360" prop="img_md">
                <el-upload class="avatar-uploader" :action="`${host}/upload/common`" :data="extra"
                    :headers="headers" :show-file-list="false" accept=".jpg,.png,.jpeg"
            ...
            <el-form-item label="商品轮播图" prop="slider">
                <el-upload v-model:file-list="fileList" :headers="headers" :action="`${host}/upload/slider`"
...
import { onBeforeUnmount, ref, shallowRef, onMounted, reactive, watchEffect, computed } from 'vue';
...
//商品分类--->还原表单：请求一级分类--->还原---->加载二级分类（还原表单）
//请求子级分类列表（未运行）
...
onMounted(async () => {
    ...
    //还原表单(获取商品详情)
    let { status, data, msg } = await Goods.goodsseller({ id: route.params.id });
    // console.log(status, data);
    //请求二级分类
    option_2nd.value = await loadSubcate(data.cate_1st);
    //请求三级分类
    option_3rd.value = await loadSubcate(data.cate_2nd);
    if (status) {
        //还原form表单
        form.value = data;
        //计算属性---根据市场价和商品价格计算折扣价
        form.value.discount = computed(() => {
            let dis = form.value.price / form.value.marketPrice;
            return (dis * 10).toFixed(2);
        });
        //显示轮播图
        image.value = form.value.slider;
        fileList.value = image.value.split(",").map((item) => {
            return { name: '1', url: item }
        })
...
//所在地：
//请求省份列表
let loadProvince = async (id) => {
    let { status, msg, data } = await Address.province({ id });
    if (status) {
        // console.log(data);
        return data;
    }
...
const rules = reactive({
    ...
    img_lg: [
        { required: true, message: '请添加主图1！', trigger: 'blur' },
    ],
    img_md: [
        { required: true, message: '请添加主图2！', trigger: 'blur' },
    ],
    slider: [
...
//上传成功1
const handleUploadSuccess1 = ({ status, msg, src }, uploadFile) => {
    //上传成功
    if (status) {
        //生成图片的地址，预览图片
...
//上传失败1
let handleUploadError1 = (error, uploadFile) => {
    //转换
...
//上传之前的检查1
const handleBeforeUpload1 = (rawFile) => {
    //判断图片格式
    ...
    formEl.validate(async (valid, fields) => {
        if (valid) {
            ...
            fileList.value.forEach((item) => {
                console.log(item);
                // 当轮播图为1张时，item下只有一个url，当照片为多个时，则会出现response下的src
                if (item.response) {
                    url.value.push(item.response.src);
                } else {
                    url.value.push(item.url);
                }
            })
            form.value.slider = url.value.join();
            console.log(form.value.slider);
            let { status, msg, data } = await Goods.edit(form.value.id, { ...form.value });
            if (status) {
                //保存成功
