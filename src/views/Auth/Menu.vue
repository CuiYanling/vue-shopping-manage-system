<template>
    <el-card class="box-card">
        ...
        <el-tree :data="menulist" default-expand-all>
            <!-- scope 解构 { node, data }  -->
    ...
    <!-- 编辑按钮 dialog 弹窗 -->
    <el-dialog v-model="edit_dialog_visible" title="编辑节点">
        ...
        <template #footer>
            <span class="dialog-footer">
                ...
                <el-button type="primary" @click="handleEdit(edit_form_ref)">保存</el-button>
            </span>
        </template>
    </el-dialog>
    <!-- 添加按钮 dialog 弹窗 -->
...
//获取菜单权限列表
onMounted(async () => {
    let { status, msg, data } = await Set.menu({ type: 'tree' });
...
let handleEdit = (formEl) => {
    formEl.validate(async (valid) => {
        ...
        if (valid) {
            let { status, msg, data } = await Set.menuedit(edit_form.value.id, { ...edit_form.value });
            if (status) {
                //编辑成功
                // console.log(data);
                //更新DOM
...
let handleAdd = (formEl) => {
    formEl.validate(async (valid) => {
        if (valid) {
            let { status, msg, data } = await Set.menuadd({ ...insert_form, component: current_data.value.component, pId: current_data.value.id, path: current_data.value.path, menu_order: current_data.value.menu_order });
            if (status) {
...
let handleRemove = (node, data) => {
    ElMessageBox.confirm('此操作将永久删除该分类，确定删除嘛？', { type: 'warning' })
        .then(async () => {
            let { status, msg } = await Set.menudelete(data.id, { id: data.id });
            if (status) {
                //删除成功，操作DOM
